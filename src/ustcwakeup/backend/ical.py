...
def get_datetime_by_period(start_period: int, end_period: int, week: int,
                           weekday: int,
                           start_date: date) -> tuple[datetime, datetime]:
    ...
    start_datetime = datetime(event_date.year,
                              event_date.month,
                              event_date.day,
                              start_time[0],
                              start_time[1],
                              tzinfo=timezone.utc) - timedelta(hours=8)
    end_datetime = datetime(event_date.year,
                            event_date.month,
                            event_date.day,
                            end_time[0],
                            end_time[1],
...
class Icalendar(CourseTableHandler, CourseTableOutput):
    ...
    def add_course(self, course: Course):
        for activity in course.activities:
            start_dt, end_dt = get_datetime_by_period(
                activity.time_period.start, activity.time_period.end,
                activity.week_range.weeks[0], activity.time_period.weekday,
                self.start_date)
            event = Event()
            for teacher in activity.teachers:
                event.add('attendee',
                          parameters={'cn': teacher.name_zh},
                          value=f'mailto:{teacher.email}')
            ...
            rrule['byday'] = ['SU', 'MO', 'TU', 'WE', 'TH', 'FR',
                              'SA'][activity.time_period.weekday]
