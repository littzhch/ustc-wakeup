...
def get_line_activities(text: str, teachers: list[Teacher]) -> list[Activity]:
    ...
    possible_names = [teacher.name_zh for teacher in teachers
                      ] + [teacher.name_en for teacher in teachers]
    teacher_name = difflib.get_close_matches(teacher_name,
                                             possible_names,
                                             n=1,
                                             cutoff=0.0)[0]
    ...
    for course_data in data["lessons"]:
        ...
        for teacher_data in course_data["teacherAssignmentList"]:
            ...
            if teacher_data["teacher"]["title"]:
                ...
                teacher.title = ""
            teacher.gender = teacher_data["teacher"]["person"]["gender"][
                "id"] % 2
            course.teachers.append(teacher)

        activity_text = course_data["scheduleText"]["dateTimePlacePersonText"][
            "textZh"]
        if activity_text:
