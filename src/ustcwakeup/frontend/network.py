...
async def get_web_data_async(username: str, 
    ...
    async with aiohttp.ClientSession(connector=connector, headers=header) as session:
        ...
        data_id_task = asyncio.create_task(session.get(semester_info_url, allow_redirects=False))
        semester_info_task = asyncio.create_task(session.get(semester_info_url))
        semester_info_task = asyncio.create_task((await semester_info_task).text())
...
def get_web_data(username: str, 
                 ...
                 force_ipv6: bool = False, 
                 force_ipv4: bool = False) -> dict:
    return asyncio.run(get_web_data_async(username, password, semester_name, force_ipv6, force_ipv4))
